...
class FastPose(nn.Module):
    ...
    def __init__(self, norm_layer=nn.BatchNorm2d, **cfg):
        ...
        # DUC = conv + bn + relu + pixel_shuffle
    ...
    def forward(self, x):
        # x.shape = torch.Size([1, 3, 256, 192])
        out = self.preact(x)
        # out.shape = torch.Size([1, 2048, 8, 6])
        out = self.suffle1(out)
        # out.shape = torch.Size([1, 512, 16, 12])
        out = self.duc1(out)
        # out.shape = torch.Size([1, 256, 32, 24])
        out = self.duc2(out)
        # out.shape = torch.Size([1, 128, 64, 48])
        out = self.conv_out(out)
        # out.shape = torch.Size([1, 17, 64, 48])
        return out
