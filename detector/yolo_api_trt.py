...
class YOLODetector(BaseDetector):
    ...
    def load_model(self):
        ...
        print('Loading tensorRT YOLO model..')

        self.model = TrtTiny(batch_size=args.detbatch, out_height=args.out_height, out_width=args.out_width,
                             engine_path=args.yolo_engine,
                             cuda_ctx=pycuda.autoinit.context,
                             dll_file=args.dll_file, mode='yolo')
    ...
    def images_detection(self, imgs, orig_dim_list):
        ...
        if not self.model:
            self.load_model()
        with torch.no_grad():
            prediction = self.model.detect_context(imgs.cpu().numpy())
            prediction = torch.from_numpy(prediction)
            prediction = prediction.to(args.device) if args else imgs.cuda()
            # ############# change 2 ########################
    ...
    def detect_one_img(self, img_name):
        ...
        with torch.no_grad():
            ...
            prediction = self.model.detect_context(img.cpu().numpy())
            prediction = torch.from_numpy(prediction)
            prediction = prediction.to(args.device) if args else img.cuda()
            # do nms to the detection results, only human category is left
            dets = self.dynamic_write_results(prediction, self.confidence,
                                              self.num_classes, nms=True,
                                              nms_conf=self.nms_thres)
